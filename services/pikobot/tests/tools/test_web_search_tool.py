...
from pikobot.agent.tools.web import WebSearchTool
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="pikobot", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("pikobot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import pikobot.agent.tools.web as web_mod
