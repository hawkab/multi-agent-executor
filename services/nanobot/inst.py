...
RABBITMQ_CHANNEL_CODE = dedent(
    ...
    class RabbitMQChannelConfig(BaseModel):
        ...
        exchange: str = "nanobot"
        exchange_type: Literal["direct", "topic", "fanout"] = "direct"
        durable: bool = True
        prefetch_count: int = 10

        inbound_queue: str = "nanobot.inbound"
        inbound_routing_key: str = "nanobot.inbound"
        outbound_routing_key: str = "nanobot.outbound"
...
DEFAULT_CONFIG = {
    ...
    "channels": {
        "rabbitmq": {
            ...
            "durable": True,
            "prefetchCount": 10,
            "inboundQueue": "nanobot.inbound",
            "inboundRoutingKey": "nanobot.inbound",
            "outboundRoutingKey": "nanobot.outbound",
            "allowFrom": ["*"],
            "streaming": False
        }
    }
...
def validate_repo_root(root: Path) -> None:
    required = [
        root / "pyproject.toml",
        root / "nanobot" / "channels" / "base.py",
        root / "nanobot" / "channels" / "registry.py",
        root / "nanobot" / "bus" / "events.py",
    ]
    missing = [str(p) for p in required if not p.exists()]
    if missing:
        fail(
            "Похоже, это не корень репозитория nanobot. Отсутствуют:\n- "
            + "\n- ".join(missing)
        )
...
def main() -> None:
    ...
    rabbitmq_channel_path = root / "nanobot" / "channels" / "rabbitmq.py"
    config_path = root / "nanobot-config" / "config.json"
    workspace_keep = root / "nanobot-config" / "workspace" / ".gitkeep"
    ...
    log("Next steps:")
    log("1. pip install -e .")
    log("2. Если используете docker compose, монтируйте ./nanobot-config в /root/.nanobot")
    log("3. Запустите nanobot gateway и проверьте обмен через RabbitMQ")
